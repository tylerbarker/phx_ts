...
export default class LiveSocket {
  constructor(url, phxSocket, opts = {}) {
    ...
    this.currentHistoryPosition =
      Number.parseInt(this.sessionStorage.getItem(PHX_LV_HISTORY_POSITION)) || 0;
  ...
  getLatencySim() {
    ...
    return str ? Number.parseInt(str) : null;
  }
  ...
  bindTopLevelEvents({ dead } = {}) {
    ...
    document.body.addEventListener("click", () => {}); // ensure all click events bubble for mobile Safari
  ...
  bindForms() {
    ...
    for (let type of ["change", "input"]) {
      this.on(type, (e) => {
        ...
        iterations++;
        let { at, type: lastType } = DOM.private(input, "prev-iteration") || {};
