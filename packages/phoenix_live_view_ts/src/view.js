...
import { clone, closestPhxBinding, isCid, isEmpty, isEqualObj, logError, maybe } from "./utils";
...
let serializeForm = (form, metadata, onlyNames = []) => {
  ...
  for (let [key, val] of formData.entries()) {
    if (onlyNames.length === 0 || onlyNames.indexOf(key) >= 0) {
      ...
      let isUnused = !inputs.some(
        (input) => DOM.private(input, PHX_HAS_FOCUSED) || DOM.private(input, PHX_HAS_SUBMITTED),
      );
...
export default class View {
  ...
  transition(time, onStart, onDone = () => {}) {
  ...
  withinTargets(phxTarget, callback, dom = document, viewEl) {
    ...
    if (isCid(phxTarget)) {
      ...
      if (targets.length === 0) {
        ...
        callback(this, Number.parseInt(phxTarget));
      }
    } else {
  ...
  maybeRecoverForms(html, callback) {
    ...
    const formsToRecover =
      ...
      DOM.all(template.content, "form")
        ...
        .filter(
          (newForm) =>
            oldForms[newForm.id].getAttribute(phxChange) === newForm.getAttribute(phxChange),
        )
        .map((newForm) => {
      ...
      let detail = {
        ...
        lock: (lockEl) => {
          return new Promise((resolve) => {
            ...
            lockEl.addEventListener(`phx:lock-stop:${newRef}`, () => resolve(detail), {
              once: true,
            });
          });
        },
  ...
  componentID(el) {
    ...
    return cid ? Number.parseInt(cid) : null;
  }
  ...
  targetComponentID(target, targetCtx, opts = {}) {
    ...
    if (isCid(cidOrSelector)) {
      return Number.parseInt(cidOrSelector);
  ...
  pushFileProgress(fileEl, entryRef, progress, onReply = () => {}) {
  ...
  pushFormSubmit(formEl, targetCtx, phxEvent, submitter, opts, onReply) {
    ...
    if (LiveUploader.hasUploadsInProgress(formEl)) {
      ...
      !(formEl.hasAttribute(PHX_REF_SRC) && formEl.classList.contains("phx-submit-loading"))
    ) {
      let meta = this.extractMeta(formEl);
      let formData = serializeForm(formEl, { submitter, ...meta });
  ...
  pushLinkPatch(e, href, targetEl, callback) {
    ...
    let refGen = targetEl
      ? () => this.putRef([{ el: targetEl, loading: loading, lock: true }], null, "click")
      : null;
      ...
      this.pushWithReply(null, "cids_will_destroy", { cids: willDestroyCIDs }).then(() => {
        ...
        this.liveSocket.requestDOMUpdate(() => {
          ...
          if (completelyDestroyCIDs.length > 0) {
            this.pushWithReply(null, "cids_destroyed", { cids: completelyDestroyCIDs }).then(
              ({ resp }) => {
